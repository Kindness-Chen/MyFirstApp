...
public abstract class ValidateTools {
    ...
    // 正则,验证是否含有特殊的字符([]^~@#%&<>{}'")
    private static final String REGEX_HAVE_SPECIAL_CHAR = "[\\[\\]\\^~@#%&<>\\{\\}'\"]+";
    ...
    // 正则,登录密码
    private static final String REGEX_LOGIN_PWD = "^(?=.*[a-zA-Z])(?=.*\\d)[\\S]{0,}$";
    ...
    private static boolean find(String string, String regex) {
        if (string == null) {
            return false;
        }
        Pattern p = Pattern.compile(regex);
    ...
    /**
     * 验证只能由A-Z a-z 0-9组成(不包括纯字母、纯数字)
     */
    public static boolean isAzazAndNum(String string) {
        return isAzazNum(string) && !(isNumOnly(string) || isLowerOrUpperCase(string));
    }

    /**
     * 验证登录密码正则
     */
    public static boolean isLoginPwd(String string) {
        return matcher(string, REGEX_LOGIN_PWD);
    }
    ...
    public static boolean isHasSoecialChar(String string) {
        return find(string, REGEX_HAVE_SOECIAL_CHAR);
    }
    ...
    public static boolean isHasSpecialChar(String string) {
        return find(string, REGEX_HAVE_SPECIAL_CHAR);
    }
