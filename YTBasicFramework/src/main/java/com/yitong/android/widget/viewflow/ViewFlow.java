...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    private static final float MOVE_TOUCHSLOP = 2f; //超过此的偏移都认为是移动而不是点击

    private static final int SNAP_VELOCITY = 1000;
    private static final int INVALID_SCREEN = -1;
    private static final int TOUCH_STATE_REST = 0;
    private static final int TOUCH_STATE_SCROLLING = 1;
    private static final int DEFAULT_TIMESPAN = 3000;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                Logs.e(TAG, "onInterceptTouchEvent  >>>  MotionEvent.ACTION_DOWN");
                if (mParent != null)
            ...
            case MotionEvent.ACTION_MOVE:
                Logs.e(TAG, "onInterceptTouchEvent  >>>  MotionEvent.ACTION_MOVE");
                if (mParent != null)
                    mParent.requestDisallowInterceptTouchEvent(true);
            ...
            case MotionEvent.ACTION_UP:
                Logs.e(TAG, "onInterceptTouchEvent  >>>  MotionEvent.ACTION_UP");
                if (mParent != null)
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                Logs.e(TAG, "onTouchEvent  >>>  MotionEvent.ACTION_DOWN");
                /*
                 * If being flinged and user touches, stop the fling. isFinished
                 * will be false if being flinged.
                 */
            ...
            case MotionEvent.ACTION_MOVE:
                Logs.e(TAG, "onTouchEvent  >>>  MotionEvent.ACTION_MOVE");
                final int deltaX = (int) (mLastMotionX - x);

                boolean xMoved = Math.abs(deltaX) > mTouchSlop;

                //---------add start  计算x移动偏移量 判断x轴是否有移动过及本次down下，是否是一次click事件
                float tempDeltaX = mLastMotionX - ev.getX();
                boolean isXMoved = Math.abs(tempDeltaX) > MOVE_TOUCHSLOP;
                boolean tempIsMoved = isXMoved;//x有点偏移都认为不是点击事件
            ...
            case MotionEvent.ACTION_UP:
                Logs.e(TAG, "onTouchEvent  >>>  MotionEvent.ACTION_UP");
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    final VelocityTracker velocityTracker = mVelocityTracker;
                    velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
                    int velocityX = (int) velocityTracker.getXVelocity();
                ...
                // 若是一次click，则执行点击。这里仿AbsListView 采用PerformClick---------add start
                if (mIsClick) {
